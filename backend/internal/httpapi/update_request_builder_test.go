...
func TestBuildUpdateRequestForContainer_IncludesProjectAndGitOpsOverrideEnvFiles(t *testing.T) {
	...
	if len(res.Request.ComposeEnvFiles) != 2 {
		t.Fatalf("expected 2 compose env files, got %#v", res.Request.ComposeEnvFiles)
	}
	if res.Request.ComposeEnvFiles[0] != envPath {
		t.Fatalf("expected project env first, got %#v", res.Request.ComposeEnvFiles)
	}
	wantManaged := filepath.Join(root, ".harborwatch", "gitops-env", "src-1", "dep-1.env")
	if res.Request.ComposeEnvFiles[1] != wantManaged {
		t.Fatalf("expected managed override env second, got %#v", res.Request.ComposeEnvFiles)
	}
}
